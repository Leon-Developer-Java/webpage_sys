...
test("the standalone ERA5 WebGL particle component compiles as a Vue SFC", () => {
  ...
  assert.ok(script.content.includes("default: 0.84"));
  assert.ok(script.content.includes("visibility = mix(0.76, 1.0"));
  ...
  assert.ok(script.content.includes("previousLon"));
  assert.equal(script.content.includes("gl.POINTS"), false);
